        ...
        <AnimatePresence mode="wait">
            <div className="fixed inset-0 z-[999] flex items-center justify-center">
                ...
                <motion.div
                    ...
                    className="relative z-[100] w-full max-w-lg sm:max-w-2xl mx-4 flex flex-col bg-card border border-border rounded-[24px] sm:rounded-3xl shadow-2xl overflow-hidden"
                >
                    <div className="flex flex-col max-h-[75vh] sm:max-h-[88vh]">
                        {/* Header */}
                        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-border/40 shrink-0">
                            <div className="flex items-center gap-2.5 sm:gap-3">
                                ...
                                <div>
                                    <h3 className="text-sm font-black text-foreground tracking-tight uppercase">Generate with AI</h3>
                                    <p className="text-[10px] text-foreground/40 font-bold truncate max-w-[150px] sm:max-w-none">{eventTitle}</p>
                                </div>
                            </div>
                            <button
                                onClick={onClose}
                            ...
                            {step === "preview" && (
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-3">
                                        <button
                                            onClick={handleReset}
                                            className="flex-1 py-3.5 sm:py-3 rounded-[14px] text-xs font-black uppercase tracking-widest bg-secondary text-foreground/60 hover:bg-foreground/10 transition-colors"
                                        >
                                            Regenerate
                                        </button>
                                        <button
                                            onClick={handleSubmit}
                                            className="flex-1 py-3.5 sm:py-3 rounded-[14px] text-xs font-black uppercase tracking-widest bg-primary text-white hover:bg-primary-dark active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                                        >
