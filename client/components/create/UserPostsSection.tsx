...
import { Heart, Clock, FileImage } from "lucide-react";
...
interface PostedSubmission {
    ...
    createdAt: string;
    _count?: { votes: number };
    event?: { id: string; title: string; imageUrl?: string; imageCid?: string; status?: string };
}
        ...
        <section>
            ...
            <div className="flex gap-3 overflow-x-auto pb-1 -mx-4 px-4 sm:mx-0 sm:px-0 no-scrollbar">
                {loading
                    ...
                    : posts.map((post, i) => (
                        <Link key={post.id} href={`/events/${post.eventId}`} className="block flex-shrink-0 w-[160px] sm:w-[176px]">
                            <motion.div
